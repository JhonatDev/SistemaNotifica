...
LOCAL_BACKUP_DIR="/tmp/backup_${SERVERTYPE}_${BACKUP_DATETIME}"
...
case "$SERVERTYPE" in
    frontend)
        if [ -d /opt/frontend ]; then
            tar czf "$LOCAL_BACKUP_DIR/frontend_static_${BACKUP_DATETIME}.tgz" -C /opt/frontend .
    ...
    backend)
        if [ -d /opt/backend ]; then
            tar czf "$LOCAL_BACKUP_DIR/backend_files_${BACKUP_DATETIME}.tgz" -C /opt/backend .
        ...
        if [ -d /etc/local.d ]; then
            tar czf "$LOCAL_BACKUP_DIR/locald_${BACKUP_DATETIME}.tgz" -C /etc/local.d .
        ...
        if [ -d /root/SpringNotifica/Notifica/src/main/resources/static/image/download ]; then
            tar czf "$LOCAL_BACKUP_DIR/images_${BACKUP_DATETIME}.tgz" -C /root/SpringNotifica/Notifica/src/main/resources/static/image/download .
        ...
        ;;
    db)
        DB_DUMP_FILE="$LOCAL_BACKUP_DIR/db_dump_${BACKUP_DATETIME}.tgz"
