...
var AutofillMonitor = class _AutofillMonitor {
  ...
  monitor(elementOrRef) {
    ...
    const result = new import_rxjs2.Subject();
  ...
  ngAfterViewInit() {
    if (this._platform.isBrowser) {
      ...
      this._ngZone.runOutsideAngular(() => {
        ...
        (0, import_rxjs2.fromEvent)(window2, "resize").pipe((0, import_operators2.auditTime)(16), (0, import_operators2.takeUntil)(this._destroyed)).subscribe(() => this.resizeToFitContent(true));
...
var MdbFormControlComponent = class _MdbFormControlComponent {
  ...
  ngAfterContentInit() {
    ...
    if (this._label) {
      this._contentObserver.observe(this._label.nativeElement).pipe((0, import_operators3.takeUntil)(this._destroy$)).subscribe(() => {
        this._updateBorderGap();
      });
    }
    this._formControl.stateChanges.pipe((0, import_operators3.takeUntil)(this._destroy$)).subscribe(() => {
      this._updateLabelActiveState();
      if (this._label) {
        this._updateBorderGap();
      }
    ...
    this._ngZone.runOutsideAngular(() => {
      this._ngZone.onStable.pipe((0, import_operators3.takeUntil)(this._destroy$)).subscribe(() => {
